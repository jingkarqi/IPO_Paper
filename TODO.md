...
- [ ] 确认研究窗口是否以 `2019-2023` 为主样本。
- [ ] 确认样本范围是否覆盖全部 A 股 IPO，还是先用注册制板块作为技术性降维样本。
- [ ] 明确“上市后第一年”口径。
  - 建议写清是“IPO 后第一个完整会计年度”还是“上市后自然年”。
- [ ] 明确行业分类口径。
...
- [ ] 统一论文和代码中的指标定义。
  - 当前代码中的 `auth_index` 公式是 `0.3*mention_z + 0.4*scene_z + 0.3*prudence_z`。
  - 当前论文 `paper/sections/04_design.tex` 与 `paper/sections/appendix.tex` 仍写成 `0.20/0.45/0.35`。
